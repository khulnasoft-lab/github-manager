...
postgresql_database_user 'github-connector-database-user' do
  ...
  username node['github_manager']['db']['user']
  password GithubManager::Helpers.database_password(node)
end

# Create database
postgresql_database 'github-connector-database' do
  connection postgresql_connection_info
  database_name node['github_manager']['db']['name']
  owner node['github_manager']['db']['user']
end
