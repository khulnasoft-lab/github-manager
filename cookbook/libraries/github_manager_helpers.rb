  ...
  class Helpers
    class << self
      ...
      def secret(key, default, node)
        data_bag = GithubManager::Helpers.load_data_bag(
          node['github_manager']['secrets_databag'],
          node['github_manager']['secrets_databag_item']
        ) rescue nil
        ...
        unless Chef::Config[:solo]
          node.set_unless['github_manager']['secrets'][key] = default
          node.save
        end

        raise "Must set github_manager.secrets.#{key}!" unless node['github_manager']['secrets'][key]

        node['github_manager']['secrets'][key]
      end
    end
  end
end
